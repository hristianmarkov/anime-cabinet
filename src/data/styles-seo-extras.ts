...
export const styleSeoExtras: Record<string, string> = Object.fromEntries(
  allStyles.map((s) => {
    ...
    return [
      s.slug,
      `Looking for a ${primary}? Anime Cabinet creates personalised ${s.name} artwork from your photos — ideal as a custom anime poster gift, wall art, or anniversary surprise. Popular searches include ${secondary}. Order online in minutes: upload your photo, pick your scene, and our artists deliver a preview within 48 hours with unlimited free revisions.`,
    ];
  })
);
