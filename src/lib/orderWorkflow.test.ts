...
test("physical orders before review keep both branches upcoming", () => {
  ...
  assert.deepEqual(pipeline.branches.shipping?.map((step) => step.state), ["upcoming", "upcoming", "upcoming", "upcoming"]);
});
...
test("a physical order can complete its digital branch independently", () => {
  ...
  assert.deepEqual(pipeline.branches.digital.map((step) => step.state), ["done", "done"]);
  assert.deepEqual(pipeline.branches.shipping?.map((step) => step.state), ["current", "upcoming", "upcoming", "upcoming"]);
});
...
test("a physical order can complete both fulfillment branches", () => {
  ...
  assert.deepEqual(pipeline.branches.digital.map((step) => step.state), ["done", "done"]);
  assert.deepEqual(pipeline.branches.shipping?.map((step) => step.state), ["done", "done", "done", "done"]);
});
